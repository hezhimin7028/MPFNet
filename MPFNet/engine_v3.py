...
class Engine():
    def __init__(self, args, model, optimizer, scheduler, loss, loader, ckpt):
        ...
        # add path loader
        self.query_path = loader.queryset.query
        self.gallery_path = loader.galleryset.gallery
    ...
    def test(self):
        def my_plt(PATHS):
            ...
            with open('my_submition.json', 'w') as f:
                json.dump(result, f)
        ...
        self.ckpt.write_log('\n[INFO] Test:')
        self.model.eval()
        gallery_paths = [item[0] for item in self.gallery_path]
        query_paths = [item[0] for item in self.query_path]
        ...
        r, m_ap, path = evaluation(
            dist, query_ids, gallery_ids, query_cams, gallery_cams, query_paths, gallery_paths, 50)
        # 生成.json文件
        my_plt(path)
        # r, m_ap = evaluation(
