...
def evaluate_py(distmat, q_pids, g_pids, q_camids, g_camids, q_paths, g_paths, max_rank):
    ...
    for q_idx in range(num_q):
        ...
        pic = list(g_paths[order[:]])
        pic = pic[:max_rank]
        pic.append(q_paths[q_idx])
...
def evaluation(
    ...
    # (distmat,q_pids,g_pids,q_camids,g_camids,max_rank=50,use_metric_cuhk03=False,use_cython=True)
    """Evaluates CMC rank.
    Args:
    ...
    """
    if use_cython and not IS_CYTHON_AVAI:
        return evaluate_cy(
            distmat, q_pids, g_pids, q_camids, g_camids, max_rank,
            use_metric_cuhk03
        )
        ...
        return evaluate_py(
            distmat, q_pids, g_pids, q_camids, g_camids, q_paths, g_paths, max_rank)
