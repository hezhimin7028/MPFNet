...
class P_rr(nn.Module):
    def __init__(self, args):
        super(P_r, self).__init__()
...
class P_rrrr(nn.Module):
    def __init__(self, args):
        super(P_rrrr, self).__init__()
        ...
        self.stand_pooling = nn.AdaptiveAvgPool2d((2, 2))
        self.sit_pooling = nn.AdaptiveAvgPool2d((3, 1))
        self.lying_pooling = nn.AdaptiveAvgPool2d((1, 3))

        self.glob_m_pooling = nn.AdaptiveMaxPool2d(1)
        self.glob_a_pooling = nn.AdaptiveAvgPool2d(1)
        # self.glob_a_pooling = nn.AdaptiveMaxPool2d(1)
    ...
    def forward(self, x):
        ...
        # tri_feat = [f_st_g[-1], f_si_g[-1], f_ly_g[-1], fuse_feat[-1]]
        tri_feat = [f_st_g[-1], f_si_g[-1], f_ly_g[-1]]

        if not self.training:
            return torch.stack([f_st_g[0], f_si_g[0], f_ly_g[0]], dim=2)
            # return torch.stack([f_st_g[0], f_si_g[0], f_ly_g[0], fuse_feat[0]], dim=2)
            # return torch.stack([ fuse_feat[0]], dim=2)
        return [f_st_g[1], f_st_0[1], f_st_1[1], f_st_2[1], f_st_3[1], f_si_g[1], f_sit_0[1], f_sit_1[1], f_sit_2[1],
                f_ly_g[1], f_ly_0[1], f_ly_1[1], f_ly_2[1]], predict_abc, tri_feat, fuse_feat[-1], fuse_feat[1]
